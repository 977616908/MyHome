...
- (void)panGestureHandler:(UIPanGestureRecognizer *)recognizer
{
    ...
    CGFloat translationY = [recognizer translationInView:_hostView].x;
    ...
    switch (recognizer.state)
    {
        ...
        case UIGestureRecognizerStateChanged:
        {
            if (_isAnimatingWithPan)
            {
                ...
                {
                    CGFloat progress = translationY / _hostView.bounds.size.width;
        ...
        case UIGestureRecognizerStateEnded:
        {
            if (_isAnimatingWithPan)
            {
                if (fabs((translationY + [recognizer velocityInView:_hostView].x / 4) / _hostView.bounds.size.width) > 0.5f)
                {
...
- (BOOL)beginFlipAnimationForDirection:(EFlipDirection)direction
{
    ...
    if (canFlipPage && currView)
    {
        ...
        _bgTopLayer.contentsGravity = kCAGravityLeft;
        _bgBottomLayer.contentsGravity = kCAGravityRight;
        _flipFrontSubLayer.contentsGravity = kCAGravityLeft;
        _flipBackSubLayer.contentsGravity = kCAGravityRight;
        
//        _bgTopLayer.contentsGravity = kCAGravityRight;
//        _bgBottomLayer.contentsGravity = kCAGravityLeft;
//        _flipFrontSubLayer.contentsGravity = kCAGravityRight;
//        _flipBackSubLayer.contentsGravity = kCAGravityLeft;
        switch (direction)
        {
            case kEFlipDirectionToPrePage:
            {
            ...
            case kEFlipDirectionToNextPage:
            {
                ...
                [_flipLayer setTransform:CATransform3DMakeRotation(M_PI, 0., 1., 0.)];
...
- (void)rebulidLayers
{
    ...
    _bgTopLayer.frame = CGRectMake(0.0f, 0.0f, _panelLayer.bounds.size.width/2.0, _panelLayer.bounds.size.height);
    ...
    _bgBottomLayer.frame = CGRectMake(_panelLayer.bounds.size.width/2,0, _panelLayer.bounds.size.width/2, _panelLayer.bounds.size.height);
    ...
    _flipLayer.frame = CGRectMake(0.0f, 0.0f, _panelLayer.frame.size.width/2, _panelLayer.frame.size.height);
