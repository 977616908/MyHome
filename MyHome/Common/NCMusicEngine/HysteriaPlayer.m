    ...
    if (audioSession.category != AVAudioSessionCategoryPlayback) {
        ...
        if ([device respondsToSelector:@selector(isMultitaskingSupported)]) {
            if (device.multitaskingSupported) {
                ...
                if (aError) {
                    PSLog(@"set category error:%@",[aError description]);
                }
                aError = nil;
                [audioSession setActive:YES error:&aError];
                if (aError) {
                    PSLog(@"set active error:%@",[aError description]);
                }
...
- (void) fetchAndPlayPlayerItem: (NSUInteger )startAt
{
    ...
    dispatch_async(HBGQueue, ^{
        ...
        if (blockItemGetter) {
            ...
            PSLog(@"please using setupWithGetterBlock: to setup your datasource");
            return ;
        }
        if (item == nil) {
            return ;
...
- (void)interruption:(NSNotification *)notification
{
    ...
    PSLog(@"resume playback from interruption");
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object
    ...
    if (object == audioPlayer && [keyPath isEqualToString:@"status"]) {
        if (audioPlayer.status == AVPlayerStatusReadyToPlay) {
            ...
            PSLog(@"player error!!!:%@",audioPlayer.error);
        }
    }
    ...
    if (object == audioPlayer.currentItem && [keyPath isEqualToString:@"status"]) {
        if (audioPlayer.currentItem.status == AVPlayerItemStatusFailed) {
            PSLog(@"------player item failed:%@",audioPlayer.currentItem.error);
            [self playNext];
    ...
    if(object == audioPlayer.currentItem && [keyPath isEqualToString:@"loadedTimeRanges"]){
        ...
        if (timeRanges && [timeRanges count]) {
            ...
            PSLog(@". . . %.5f  -> %.5f",CMTimeGetSeconds(timerange.start),CMTimeGetSeconds(timerange.duration));
...
//        [player pause];
//        interruptedWhilePlaying = YES;
//        PSLog(@"begin interrupting");
//    }
//}

//- (void)endInterruption
//{
//    PSLog(@"end interrupitng");
//
...
//                PSLog(@"unable to resume playback after interruption");
//                PSLog(@"%@",activationError.description);
//
//                interruptedWhilePlaying = NO;
//                manul_pause = NO;
//                [player play];
//
//            }else {
//                PSLog(@"resume from interrtuption");
//                interruptedWhilePlaying = NO;
//                manul_pause = NO;
//                [player play];
//            }
