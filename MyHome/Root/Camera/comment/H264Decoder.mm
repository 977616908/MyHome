...
NSCondition *g_Lock=[[NSCondition alloc]init];
...
int CH264Decoder::DecoderFrame(uint8_t *pbuf, int len, int &width, int &height)
{
    AVPacket avpkt;
    ...
    if(m_pCodecCtx==NULL){
        NSLog(@"CH264Decoder  DecoderFrame()  m_pCodecCtx==NULL");
        return 0;
    }
    
    if (m_pFrame==NULL) {
        NSLog(@"CH264Decoder  DecoderFrame()  m_pFrame==NULL");
        return 0;
    }
